...
class ReporteETESController extends BaseController
{
		...
		if(Auth::check()){
			...
			if($data["user"]->idrol == 1 || $data["user"]->idrol == 2 || $data["user"]->idrol == 3 || $data["user"]->idrol == 4 || 
				...
				if($id){
					...
					$data["responsable"] = User::find($data["programacion_reporte_etes"]->iduser);
				}
	...
	public function submit_create_reporte_etes(){
		if(Auth::check()){
			...
			if($data["user"]->idrol == 1 || $data["user"]->idrol == 2 || $data["user"]->idrol == 3 || $data["user"]->idrol == 4){
				...
				$attributes = array(
					'idprogramacion_reporte_etes' => 'Programaciones No Concluidas',
	...
	public function return_num_doc_responsable_etes(){
