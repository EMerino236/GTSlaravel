...
@section('content')
	<div class="row">
        <div class="col-lg-12">
            <h3 class="page-header">Editar Proceso: <strong>{{$documento_info->nombre}}</strong></h3>
        </div>
        <!-- /.col-lg-12 -->
    </div>
	...
	{{ Form::open(array('url'=>'mapa_procesos/edit_proceso/'.$documento_info->iddocumentosinf, 'role'=>'form', 'files'=>true)) }}
		...
		<div class="container-fluid row">			
			...
			@endif
			<div class="col-md-2 form-group">
				<a class="btn btn-default btn-block" href="{{URL::to('/mapa_procesos/list_procesos')}}">Cancelar</a>
			</div>	
		{{ Form::close() }}
			@if($documento_info->deleted_at)
			{{ Form::open(array('url'=>'mapa_procesos/submit_enable_proceso', 'role'=>'form')) }}
				{{ Form::hidden('documento_id', $documento_info->iddocumentosinf) }}
			...
			{{ Form::close() }}
			@else
			{{ Form::open(array('url'=>'mapa_procesos/submit_disable_proceso', 'role'=>'form')) }}
				{{ Form::hidden('documento_id', $documento_info->iddocumentosinf) }}
