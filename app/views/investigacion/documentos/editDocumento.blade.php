...
@section('content')
	...
	{{ Form::open(array('url'=>'documento_investigacion/submit_edit_documento', 'role'=>'form', 'files'=>true)) }}
		...
		<div class="container-fluid row">			
			...
			@endif
			<div class="col-md-2 form-group">
				<a class="btn btn-default btn-block" href="{{URL::to('/documento_investigacion/list_documentos')}}">Cancelar</a>
			</div>	
		{{ Form::close() }}
			@if($documento_info->deleted_at)
			{{ Form::open(array('url'=>'documento_investigacion/submit_enable_documento', 'role'=>'form')) }}
				{{ Form::hidden('documento_id', $documento_info->iddocumentosinf) }}
			...
			{{ Form::close() }}
			@else
			{{ Form::open(array('url'=>'documento_investigacion/submit_disable_documento', 'role'=>'form')) }}
				{{ Form::hidden('documento_id', $documento_info->iddocumentosinf) }}
